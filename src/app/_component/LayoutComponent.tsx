...
const menuItems = [
  ...
  {
    path: '/snippets/14',
    label: 'Client 컴포넌트를 서버 컴포넌트로 감싸서 구현하기',
  },
  {
    path: '/snippets/15',
    label: '[TanStack] prefetchQuery, useQuery 사용하기',
  },
  {
    path: '/snippets/16',
    label: '[TanStack] prefetchQuery 중복 호출 staleTime 확인',
  },
  { path: '/snippets/17', label: '[TanStack] HydrateBoundary 영역 테스트' },
  {
    path: '/snippets/18',
    label: '[TanStack] prefetchQuery, useQuery 정상 동작 구현',
  },
  { path: '/snippets/19', label: '[Error Boundary] Client Comp 에러 구현' },
  { path: '/snippets/20', label: '[Error Boundary] Server Comp 에러 구현' },
  {
    path: '/snippets/21',
    label: '[Error Boundary] Client Comp fetch 에러 구현',
  },
  {
    path: '/snippets/22',
    label: '[Error Boundary] Server Comp fetch 에러 구현',
  },
  { path: '/snippets/23', label: '[useAxios] axios 공통 모듈 구현' },
  { path: '/snippets/24', label: '[useAxios] axios intersection observer' },
  {
    path: '/snippets/25',
    label: '[TanStack] useInfinityQuery intersection observer',
  },
  ...
  {
    path: '/snippets/53',
    label: '[await import,tanstack] 컴포넌트 지연 로딩(scroll) 2',
  },
  {
    path: '/snippets/54',
    label: '[await import,tanstack] 컴포넌트 지연 로딩(scroll) 3',
  },
  ...
  {
    path: '/snippets/58',
    label: '[swiper, tab] 메인 페이지 tab swiper 예시(미구현)',
  },
  { path: '/snippets/59', label: '재귀 컴포넌트 예시' },
  { path: '/snippets/60', label: 'Shadow Dom 을 사용한 CSS 격리' },
  {
    path: '/snippets/61',
    label: 'Shadow Dom 을 사용한 외부 Service 렌더링(미구현)',
  },
  {
    path: '/snippets/62',
    label: 'library 패키징 및 배포 후 import 예시 구현(미구현)',
  },
  { path: '/snippets/63', label: '모듈 동적 import 예시' },
];
